...
- **两阶段检索 + 可量化评测**：向量召回后接 cross-encoder 重排取 top-k，用公开数据集 CMRC2018 的评测脚本，以 Recall / MRR / nDCG 量化重排收益与延迟代价。
...
评测直接用**公开学术数据集 CMRC2018**（哈工大讯飞发布的中文机器阅读理解数据集），而非自造语料——数据、问题、答案标注都由第三方给定，可追溯、可复现，避免"自己编数据又自己判分"。构建脚本把它复用为检索评测：所有不重复段落当作知识库，每个问题的正确来源即它自带的那段 context，检索任务就是从整个语料库里把对应段落捞进 top-k。

实测结果（公开数据集 CMRC2018，语料 150 段 / 问题 300 条，纯离线可复现）：
...
| MRR | 0.980 | 0.997 |
| nDCG@10 | 0.983 | 0.997 |
| 单次检索延迟 p50 | 4.1ms | 1622.9ms |

重排把首位命中率从 97.0% 提到 99.7%（MRR 0.980→0.997），代价是每次多约 1.6s 的 CPU 精排。分数偏高是数据特性：CMRC2018 段落取自维基百科不同词条，主题分散、语义区分度大，向量召回本身已经很强——公开数据集难度由数据本身决定，不可挑选。每题只对应一段（单正例），因此 Recall@k 等于命中率、理想 nDCG 上界为 1，这些局限在报告里如实标注。

```bash
# 首次：从 HuggingFace 下载 CMRC2018 并构建评测集（app/eval/data/cmrc_eval.json）
...
backend/
  app/
    ...
    eval/           # 检索评测:CMRC2018 数据集构建 + Recall/MRR/nDCG 脚本
