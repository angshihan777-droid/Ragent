...
<template>
  <section class="manager">
    <header class="heading"><div><h1>{{ allProjects ? '知识库图书馆' : (store.currentProject?.name ? store.currentProject.name + ' · 项目资料' : '项目资料') }}</h1><p>在这里上传、查看、编辑或删除资料，修改会同步更新检索索引。</p></div></header>
    <div class="toolbar">
      ...
      <div class="tools"><button class="secondary" :disabled="busy || !projectId" @click="editor = { mode: 'create' }">新建文本</button><button :disabled="busy || !projectId" @click="picker.click()">{{ busy ? '处理中…' : '上传资料' }}</button></div>
    ...
    <div v-if="error" class="error" role="alert">{{ error }} <button class="secondary" :disabled="busy || loading" @click="reload">刷新列表</button></div>
    <div class="list-head"><span>{{ shown.length }} 份资料</span><button class="secondary" :disabled="busy || loading" @click="reload">刷新</button></div>
...
<style scoped>
.manager { width: 100%; max-width: 1040px; margin: 0 auto; padding: 32px; box-sizing: border-box; }.heading h1 { margin: 0; font-size: 23px; letter-spacing: -.5px; }.heading p { margin: 10px 0 24px; color: var(--muted); font-size: 13px; line-height: 1.8; }.toolbar { display: flex; gap: 12px; align-items: center; flex-wrap: wrap; }.toolbar select { width: auto; min-width: 160px; max-width: 260px; }.search { flex: 1; width: auto; min-width: 160px; }.tools { display: flex; gap: 8px; }.tools button { white-space: nowrap; }.hint { font-size: 12px; color: var(--muted); line-height: 1.8; margin: 12px 0 24px; }.secondary { background: var(--panel); color: var(--ink); border: 1px solid var(--border); }.secondary:hover:not(:disabled) { background: var(--panel2); }.list-head { display: flex; align-items: center; justify-content: space-between; margin-bottom: 12px; font-size: 13px; color: var(--muted); }.list-head button { padding: 5px 10px; font-size: 12px; }.notice { color: var(--accent-d); font-size: 13px; }.error { padding: 12px; margin-bottom: 16px; background: #fff4f4; color: #ad3737; font-size: 13px; line-height: 1.8; overflow-wrap: anywhere; }
@media (max-width: 600px) { .manager { padding: 20px 14px; }.tools { width: 100%; justify-content: flex-end; }.toolbar select { max-width: 100%; } }
</style>
